...
export class Posts extends React.Component {
    ...
    spotifyCode = (spotifyUrl) => {
        ...
        if (spotifyUrl.search('playlist') != '-1') {
            ...
            if ((matchUser && matchUser[2].length == 11) && (matchCode && matchCode[2].length == 22)) {
                return ' https://embed.spotify.com/?uri=spotify:user:' + matchUser[2]+ ':playlist:' + matchCode[2];
            } else {
                return null;
            }
        }
        ...
        if (url && url.toLowerCase().search('youtube') != '-1') {
            return (
                <div className='post-media'>
                    <iframe width='100%' height='400px' src={'https://www.youtube.com/embed/' + this.youtubeCode(url) + '?showinfo=0'} frameBorder='0' allowFullScreen></iframe>
                </div>)
        } else if (url && url.toLowerCase().search('spotify') != '-1') {
        return (
            <div className="spotify-embeds">
                <div className="spotify-embed">
                    <iframe src={this.spotifyCode(url)} width="100%" height="80" frameborder="0" allowtransparency="true"></iframe>
                </div>
            </div>
        )
        }
                ...
                {this.props.data.map((post, index) => (
                    <div key={'post_' + index} className='post-container'>
                        <div className='post-card'>
                            ...
                            {post.url && post.url !== null && post.url !== undefined && this.renderMedia(post.url)}
