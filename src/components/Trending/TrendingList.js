...
export class TrendingList extends React.Component {
  ...
  renderFormContainer = () => {
    return (
      <div className='form-container mt-20'>
        <input className='form-input' name='url' value={this.url} onChange={this.handleInputChange} placeholder='http://www.youtube...' />
        <div className='youtube-icon'><i className="fab fa-youtube"></i></div>
        <textarea className='form-textarea mt-10' name='post' value={this.post} onChange={this.handleInputChange} placeholder="Hey, tell us what you've been listening!"></textarea>
        <button className='primary-button' onClick={this.postTweet} type='button'>Publish</button>
        <div style={{ clear: 'both' }}></div>
        {this.showSuccessPost && <MessageBox type='success' message='Item posted successfully!' show={() => this.showSuccessPost = false} />}
        {this.showInfoPost && <MessageBox type='info' message='Please insert a Youtube URL or post a comment!' show={() => this.showInfoPost = false} />}
      </div>
    )
  }
  ...
  startSocket = () => {
    this.socket.on("tweet", data => {
      if (data.user && data.user.location && data.user.location !== undefined && data.user.location.toLowerCase().search(this.TrendingController.currentCity.toLowerCase()) != '-1') {
  ...
  setInfiniteScroll = () => {
    return window.addEventListener('scroll', () => {
      if (document.body.scrollTop + document.body.clientHeight >= document.body.scrollHeight) {
        ...
        this.TrendingController.getNearbyPosts(this.coords, lastId).then((result) => {
          result.data.statuses.map(status => this.posts.push(this.TrendingController.formatPost(status)))
        })
      }
    });
  }
